...
def plot_clustered_images(emb_2d, labels, names, prices_list, out_file, title_str, show_outliers=False, cluster_name_map=None, clean=True):
    """
    ...
    for i, label in enumerate(unique_labels):
        ...
        nice = ""
        if cluster_name_map and (label in cluster_name_map):
            nice = f" · {cluster_name_map[label]}"
        cluster_ax.set_title(
            f"Cluster {label} (n={n_images}){nice}{stats_text}",
            loc='left',
            fontsize=14,
            pad=10
        )
...
#def enforce_min_cluster_size(X, labels, min_size=6, strategy="outlier"):
    """
    X: (n, d) embeddings
    labels: (n,) cluster labels from Agglomerative
    strategy: "outlier" -> set small clusters to -1
              "merge"   -> merge small clusters to nearest large-cluster centroid
    """
    ...
    # merge strategy
    # 1) compute centroids
    centroids = {c: X[labels == c].mean(axis=0) for c in counts.keys()}
    big = [c for c, sz in counts.items() if sz >= min_size]
    if not big:
        # everything is small; leave as is or mark all as outliers
        labels[:] = -1
        return labels

    big_centroids = np.stack([centroids[c] for c in big], axis=0)

    # 2) reassign points from each small cluster to nearest big centroid
    for c in small:
        idx = np.where(labels == c)[0]
        pts = X[idx]
        d = ((pts[:, None, :] - big_centroids[None, :, :]) ** 2).sum(axis=2)  # squared euclid
        nearest_big = np.argmin(d, axis=1)
        target_labels = np.array(big)[nearest_big]
        labels[idx] = target_labels
    return labels
...
def _quick_descriptors(img_pil):
    """
    Very fast, robust-ish cues:
      - aspect_ratio, circularity: from binary mask
      - edge_density: proxy for thin metal vs thick acetate
      - mean_brightness, mean_saturation
      - tortoise_score: brownish + high variance (very heuristic!)
    """
    img = np.array(img_pil.convert("RGB"))
    h, w = img.shape[:2]
    # center crop (avoid background text/edges)
    pad = int(min(h, w) * 0.08)
    img_c = img[pad:h-pad, pad:w-pad].copy()
    if img_c.size == 0:
        img_c = img
    ...
    mean_brightness = float(V.mean())/255.0
    mean_saturation = float(S.mean())/255.0

    # Simple foreground via adaptive threshold on V
    v_blur = cv2.GaussianBlur(V, (5,5), 0)
    thr = cv2.adaptiveThreshold(v_blur.astype(np.uint8), 255,
    ...
    if cnts:
        ...
        aspect_ratio = float(w2)/(h2+1e-6)
    else:
        circularity, aspect_ratio = 0.4, 2.0

    # Edges (proxy for “thin metal” if edges >> filled area)
    edges = cv2.Canny(cv2.cvtColor(img_c, cv2.COLOR_RGB2GRAY), 80, 160)
    edge_density = float(edges.mean())  # 0..255 scaled later

    # Brownish / tortoise-ish score (R and G high, B lower, and variance present)
    R, G, B = img_c[:,:,0].astype(np.float32), img_c[:,:,1].astype(np.float32), img_c[:,:,2].astype(np.float32)
    brown_level = np.clip((R+G)/2.0 - B, 0, 255).mean()
    color_var = img_c.reshape(-1,3).astype(np.float32).var(axis=0).mean()
    tortoise_score = float(brown_level/255.0 * (color_var/1000.0))

    # Lens darkness via central horizontal band (glasses usually centered)
    band_h = max(4, img_c.shape[0]//6)
    mid = img_c.shape[0]//2
    band = V[mid-band_h//2:mid+band_h//2, :]
    lens_darkness = float(1.0 - band.mean()/255.0)

    return dict(
        aspect_ratio=aspect_ratio,
        circularity=circularity,
        edge_density=edge_density/255.0,  # 0..1
        mean_brightness=mean_brightness,
        mean_saturation=mean_saturation,
        tortoise_score=tortoise_score,
        lens_darkness=lens_darkness
    )

def _name_from_stats(stats):
    """
    Turn aggregated stats into a compact human label: Shape + (Material) + [Lens/Color].
    """
    ar = stats['aspect_ratio']
    circ = stats['circularity']
    edge = stats['edge_density']
    sat = stats['mean_saturation']
    dark = stats['lens_darkness']
    tort = stats['tortoise_score']

    # ---- SHAPE ----
    if ar >= 2.2:
        shape = "Slim Rectangle (’90s)"
    elif 1.6 <= ar < 2.2:
        shape = "Rectangular"
    elif 1.25 <= ar < 1.6:
        shape = "Soft Rectangle / Oval"
    elif circ >= 0.7:
        shape = "Round / Panto"
    else:
        shape = "Geometric"

    # ---- MATERIAL FEEL (very heuristic) ----
    # thin metal often ⇒ higher edge density, lower saturation
    material = "Metal" if (edge > 0.08 and sat < 0.35) else "Acetate"

    # ---- LENS/COLOR TAGS ----
    lens = "Blackout Lenses" if dark > 0.5 else "Light/Gradient Lenses"
    color_tag = "Tortoiseshell" if tort > 0.12 else None
...
def auto_name_clusters(labels, names, image_lookup):
    """
    labels: array-like cluster ids (after any relabeling)
    names:  list of image stems aligned with labels
    image_lookup: dict stem -> PIL image (preprocessed_images)
    returns: dict {cluster_id: "Auto Name"}
    """
    by_cluster = defaultdict(list)
    for l, n in zip(labels, names):
        if l == -1: 
            continue
        if n in image_lookup:
            by_cluster[l].append(_quick_descriptors(image_lookup[n]))

    name_map = {}
    for cid, lst in by_cluster.items():
        if not lst:
            name_map[cid] = f"Cluster {cid}"
            continue
        # aggregate
        agg = {k: float(np.mean([d[k] for d in lst])) for k in lst[0].keys()}
        name_map[cid] = _name_from_stats(agg)
    return name_map
...
def run_agglomerative(X, names, prices, out_file="clusters_agglomerative.png", title="Agglomerative Clustering"):
    ...
    if best_k is not None:
        ...
        new_labels, ranked, stats = reorder_labels_by_separation(
         ...
         # 🔹 NEW: auto-name clusters from images
        global preprocessed_images
        cluster_name_map = auto_name_clusters(new_labels, names, preprocessed_images)
