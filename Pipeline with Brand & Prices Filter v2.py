...
# ★ Cargar todas las 5 marcas como solicitado
...
color_embs_full = []
shape_embs_full = []
names_full = [] # ★ Nombres de todas las imágenes originales
vit_embs_full = []
texture_embs_full = []
...
for p in all_image_paths_list: # ★ Loop sobre TODAS las rutas de imagen
    try:
        img = load_and_preprocess(p, target=(224, 224))
        names_full.append(p.name) # ★ Guardar el nombre original del archivo
...
# --- FINAL SUMMARY WITH FILTERED DATA AND CLUSTER LABELS ---
print("\n📄 Generating final summary with filtered data and cluster labels...")

if labels_kmeans_shape_color_texture is not None and len(labels_kmeans_shape_color_texture) == len(names_for_clustering):
    cluster_results_df = pd.DataFrame({
        "product_name": [Path(n).stem for n in names_for_clustering], # ★ Usar stems de los nombres filtrados
        "cluster_id": labels_kmeans_shape_color_texture
    })

   
    final_summary_df = price_df_filtered.merge(cluster_results_df, on="product_name", how="inner")

    print(f"✅ Summary generated for {len(final_summary_df)} products.")

    out_path = "filtered_cluster_summary.csv"
    final_summary_df.to_csv(out_path, index=False, encoding="utf-8-sig")
    print(f"📄 Saved filtered cluster summary to: {out_path}")

    print("\n📊 Cluster distribution by price range:")
    cluster_stats = (
        final_summary_df.groupby(["brand", "cluster_id"])["price_eur"]
        .agg(["count", "mean", "min", "max"])
        .round(2)
    )
    print(cluster_stats)
else:
    print("\n⚠️ Skipping final summary because K-Means (Shape+Color+Texture) clustering results are not available or inconsistent for filtered data.")